...
public class MainlandPolygon {

    public static void main(String[] args) {

        System.out.println("北京:" + isInsideChinaMainland(116.22317, 39.933306));
        System.out.println("山东:" + isInsideChinaMainland(118.901702, 37.944366));
        System.out.println("朝鲜:" + isInsideChinaMainland(127.263273, 40.287348));
        System.out.println("黄海:" + isInsideChinaMainland(122.9951, 34.94996));
        System.out.println("缅甸:" + isInsideChinaMainland(97.864393, 26.994861));
        System.out.println("香港:" + isInsideChinaMainland(114.205792, 22.286119));
        System.out.println("蒙古国:" + isInsideChinaMainland(104.547211, 43.124987));
        System.out.println("乌鲁木齐:" + isInsideChinaMainland(87.511314, 43.768402));
        System.out.println("尼泊尔:" + isInsideChinaMainland(82.010522, 28.952768));
        System.out.println("日客则（西藏）:" + isInsideChinaMainland(88.90031, 29.287992));
        System.out.println("长沙:" + isInsideChinaMainland(112.963974, 28.209783));
        System.out.println("通化:" + isInsideChinaMainland(125.938668, 41.746925));
        System.out.println("大兴安岭:" + isInsideChinaMainland(123.74939, 52.496716));
        System.out.println("呼伦贝尔:" + isInsideChinaMainland(119.720382, 49.24768));
    }

    public static boolean isInsideChinaMainland(double lng, double lat) {
        int index;
        boolean inside = false;
        Point prePoint = polygon[0];
        for (index = 1; index < polygon.length; index++) {
            Point nextPoint = polygon[index];
            if (lng > Math.min(prePoint.lng, nextPoint.lng)
                    && lng <= Math.max(prePoint.lng, nextPoint.lng)
                    && lat <= Math.max(prePoint.lat, nextPoint.lat)
                    && prePoint.lng != nextPoint.lng
            ) {
                double xinters = (lng - prePoint.lng) * (nextPoint.lat - prePoint.lat) / (nextPoint.lng - prePoint.lng) + prePoint.lat;
                if (prePoint.lat == nextPoint.lat || lat <= xinters)
                    inside ^= true;
    ...
    private static final Point[] polygon = new Point[]{
